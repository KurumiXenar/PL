...
public class FunCheckerVisitor extends AbstractParseTreeVisitor<Type> implements FunVisitor<Type> {
	 ...
	 * @return the visitor result
	 */
	public Type visitBreak(FunParser.BreakContext ctx){
		if(loopCount < 1){
			reportError("break needs to be in a loop", ctx);
			return null;
		}
		return Type.VOID;
	}
	 ...
	 * @return the visitor result
	 */
	public Type visitContinue(FunParser.ContinueContext ctx){
		if(loopCount < 1){
			reportError("continue needs to be in a loop", ctx);
			return null;
		}
		return Type.VOID;
	}
